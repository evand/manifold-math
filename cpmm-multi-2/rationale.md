...
**And what survives lands in the wrong pools.** Valuing each pool's reserve delta in
expectation at add-time prices (v1's realized loss is outcome-dependent — the band above):
at a 90% favorite, only M96 of a v1 M1,000 add reaches the favorite's pool (the M807 above
is destroyed) — and even in the "safe-looking" direction, a market with one *ruled-out*
answer (traded down to q = 0.5%; dead answers commonly sit at 0.1–0.5%) still burns M140.
v2's whole-market add (the √variance merge, GP17 — creation's own allocation rule applied
to the added mana) destroys nothing in *any* resolution, and is self-allocating: each pool's
share tracks its probability, so a ruled-out answer attracts ~nothing (M2 of M1,000 at
q = 0.5%, under M1 at 0.1%). Subsidizing a market with a dead answer is, in value terms,
subsidizing the live answers — LPs don't need to think about it:
